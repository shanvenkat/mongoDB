...
var schema = new Schema ({
	...
	Kids: {type: Number, min: 0, max: 10 },
	Adults: {type: Number, min: 0, max: 10 },
...
router.post('/adduser/:firstname/:lastname/:branch/:gender/:age/:kids/:adults/:DOB/:emptype/:email', function(req, res) {
	var strfirstname = req.params.firstname;
	var strlastname = req.params.lastname;
	var strbranch = req.params.branch;
	var strgender = req.params.gender;
	var strage = parseInt(req.params.age);
	var strkids = parseInt(req.params.kids);
	var stradults = parseInt(req.params.adults);
	var strDOB = formatDate(new Date(req.params.DOB));	
	var stremptype = req.params.emptype;		 
	var stremail = req.params.email;	
	
	console.log('strkids #' +  strkids);
	console.log('stradults #' +  stradults);

	users.create({Fullname:strfirstname,Spousename:strlastname,Branch:strbranch, Gender:strgender,Age:strage, Kids:strkids, Adults:stradults, DOB:strDOB, Address:stremptype, email:stremail }, function(err){
		if (err) // ...  			
  			console.log(strfirstname + " " + strgender  + " " + strage + " " + strDOB + ' failed');
		else
		{
			console.log(strfirstname + " " + strgender  + " " + strage + " " + strDOB + " " + strbranch + " " + ' successfully added');	
...
//$http.put('/update/'+ docid + '/' +strfirstname+'/' +strlastname+'/' +strbranch+ '/' +strgender+ '/' +strage+ '/'+strkids+'/' +stradults+'/' +strdob+ '/' +stremptype+ '/' + stremail + '/')

// Update Operation using Express Router
router.put('/update/:id/:firstname/:lastname/:branch/:gender/:age/:kids/:adults/:dob/:emptype/:email', function(req, res) {
	...
	users.findOneAndUpdate({_id:strid}, {Fullname:strfirstname, Spousename:strlastname, Branch:strbranch, Gender:strgender,Age:strage, Kids:strkids, Adults:stradults, DOB:strdob,Address:stremptype,email:stremail}, function(err){
	if (err) // ...
